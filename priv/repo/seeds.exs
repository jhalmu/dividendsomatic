...
alias Dividendsomatic.Portfolio.{PortfolioSnapshot, Holding, Dividend, SoldPosition}
...
# Quarterly dividends from dividend-paying stocks
dividend_stocks = [
  %{
    symbol: "NESN",
    amounts: [Decimal.new("2.85"), Decimal.new("2.85"), Decimal.new("2.85")],
    currency: "CHF"
  },
  %{
    symbol: "EQNR",
    amounts: [Decimal.new("8.50"), Decimal.new("8.50"), Decimal.new("9.00")],
    currency: "NOK"
  },
  %{
    symbol: "MSFT",
    amounts: [Decimal.new("0.75"), Decimal.new("0.75"), Decimal.new("0.75")],
    currency: "USD"
  },
  %{symbol: "SAP", amounts: [Decimal.new("2.20"), Decimal.new("2.20")], currency: "EUR"},
  %{symbol: "OR", amounts: [Decimal.new("6.60")], currency: "EUR"}
]

# Distribute dividends across the date range
year = Date.utc_today().year

dividends_data =
  Enum.flat_map(dividend_stocks, fn stock ->
    stock.amounts
    |> Enum.with_index()
    |> Enum.map(fn {amount, idx} ->
      # Space dividends roughly monthly within the year
      month = min(idx + 1, 12)
      day = 15
      ex_date = Date.new!(year, month, day)
      pay_date = Date.add(ex_date, 14)

      # Scale by quantity
      qty = Enum.find(stocks, fn s -> s.symbol == stock.symbol end)
      total_amount = Decimal.mult(amount, qty.quantity) |> Decimal.round(2)
...
Enum.each(dividends_data, fn attrs ->
  %Dividend{}
  |> Dividend.changeset(attrs)
  |> Repo.insert!()
end)

IO.puts("Created #{length(dividends_data)} dividend records.")
...
sold_positions_data = [
  %{
    ...
    purchase_price: Decimal.new("280.00"),
    purchase_date: ~D[2025-06-15],
    sale_price: Decimal.new("450.00"),
    sale_date: ~D[2025-11-20],
    currency: "USD",
    notes: "Took profits after strong AI rally"
  },
  %{
    symbol: "BABA",
    description: "Alibaba Group Holding",
    quantity: Decimal.new("50"),
    purchase_price: Decimal.new("120.00"),
    purchase_date: ~D[2025-03-10],
    sale_price: Decimal.new("88.00"),
    sale_date: ~D[2025-09-05],
    currency: "USD",
    notes: "Cut losses on China regulatory concerns"
  },
  %{
    symbol: "SHELL",
    description: "Shell PLC",
    quantity: Decimal.new("100"),
    purchase_price: Decimal.new("28.50"),
    purchase_date: ~D[2025-01-20],
    sale_price: Decimal.new("32.80"),
    sale_date: ~D[2025-08-12],
    currency: "EUR",
    notes: "Rotated into renewable energy plays"
  }
]
